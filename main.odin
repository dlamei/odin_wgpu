...
main :: proc() {
    ...
    on_device :: proc "c" (status: wgpu.RequestDeviceStatus, device: wgpu.Device, message: string, userdata1: rawptr, userdata2: rawptr) {
        ...
        width, height: u32 = os_get_framebuffer_size()
...
resize :: proc "c" () {
    ...
    state.config.width = max(state.config.width, 1)
    state.config.height = max(state.config.height, 1)
    wgpu.SurfaceConfigure(state.surface, &state.config)
    frame()
}
...
frame :: proc "c" () {
    ...
    dt := f32(time.duration_seconds(time.tick_since(last_frame_time)))
